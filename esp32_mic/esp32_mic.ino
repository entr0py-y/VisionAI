...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    // CRITICAL: If the ESP32 misses the router's beacon on the first attempt,
    // it natively gives up internally! The standard `while` loop will just
    // print `.` forever. We strictly enforce an aggressive 10-second timeout to
    // brutally kickstart the modem again!
    if (wifiRetries >= 20) {
      Serial.println("\n[WIFI] Scan Timeout! Hotspot likely asleep or hiding. "
                     "Forcing RF Antenna Reboot...");
      WiFi.disconnect(true);
      delay(500);
      WiFi.begin(ssid, password);
      wifiRetries = 0;
  ...
  i2s_config_t i2s_config = {
      ...
      .sample_rate = 16000,
      .bits_per_sample =
          I2S_BITS_PER_SAMPLE_32BIT, // Must read 32-bit to capture INMP441's
                                     // 24-bit precision!
  ...
  // This explicitly guarantees it sits perfectly at LOW (0V) instead of
  // floating into an infinite loop!
...
void loop() {
  ...
  HTTPClient http;
  http.setReuse(true);
  String statusUrl = String("https://") + serverIp +
                     "/api/pi/status?device=mic&t=" + String(millis());
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (payload.indexOf("\"RECORD\"") >= 0 || payload.indexOf("RECORD") >= 0) {
      Serial.println("\n[TRIGGER] Received remote RECORD command. Simulating "
                     "standard pulse...");
...
void recordToRAMAndSend(bool isRemotelyTriggered) {
  ...
  // This physically bypasses the 1.5-second mbedTLS handshaking latency
  // entirely!
  ...
  // We can easily stream up to 500,000 bytes (~15.6 seconds) seamlessly now
  // that we bypassed the RAM arrays!
  ...
  while (totalBytesRecorded < maxDurationBytes) {
    // DYNAMIC PTT FAST-PATH: If the user releases the physical button,
    // violently snap the recording off early! This trivially slashes exactly as
    // many seconds of latency off the backend processing as the user didn't
    // need to use! We physically enforce a minimum of 0.5 seconds (16,000
    // bytes) so the Cloud STT API doesn't reject an empty file.
    if (digitalRead(TOUCH_PIN) == LOW && totalBytesRecorded > 16000) {
      Serial.println("👆 Physical Button Released! Cutting stream early to "
                     "reduce massive API latency...");
      break;
    }
    ...
    // HTTP Chunked streaming format natively encapsulates fragments
    // dynamically!
  ...
  digitalWrite(LED_BUILTIN,
               LOW); // PHYSICAL INDICATOR: TURN OFF (Stop Speaking)

  Serial.printf("⏹️ LIVE STREAM CONCLUDED. Securely beamed %u bytes!\n",
                totalBytesRecorded);
  Serial.println(
      "✅ Audio successfully bridged to Cloud Transcriber. Waiting for API...");
